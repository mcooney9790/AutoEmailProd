        ...
        try:
            ...
            while i < len(df):
                try:
                    ...
                    myemail = 'stephen.buzby@thepropertyrp.com'
                    email_subject = replace_html(brand_series[i],name_series[i], subject)
                    email_body = replace_html(brand_series[i], name_series[i], body)

                    # Formulate message
                    msg_content = build_message(email_series[i],myemail,email_subject,email_body)

                    # Attach files
                    msg = attach_files(attach_path ,msg_content,attachments)

                    send_gmail(msg,'stephen.buzby@thepropertyrp.com','pandas89',myemail, email_series[i])
